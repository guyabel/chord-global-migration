...
pdf(file = "./plot-ims2024/time_totals.pdf", width = 7, height = 3)
for(i in 1:nrow(d3)){
  y0 <- d3 %>%
    slice(i) %>%
    pull(year) %>%
...
a <- image_read_pdf(path = "./plot-ims2024/time.pdf")
s <- image_read_pdf(path = "./plot-ims2024/time_totals.pdf")
# image_info(a)
...
#       image_annotate(
#         text = "
# The arrowheads shows the 
# direction of migration. 
# Width represents the size of 
# the migrant population. 
# Numbers on the sector axis indicate
# the migrant population size in millions. 
# Axis limits are set by the maximum 
# values across both sexes" ,
#         size = 8, color = "grey30",
...
Data from United Nations Department of Economic and Social Affairs, Population Division (2024). International Migrant Stock 2024.",
  ...
  video.name = "./plot-ims2024/time.mp4"
)
file.show("./plot-ims2024/time.mp4")
...
library(av)
av_encode_video(
  input = "./plot-ims2024/time.mp4",
  output = "./plot-ims2024/time.gif",
)
file.show("./plot-ims2024/time.gif")
